...
class SM_Project_Stage
{
    ...
    {
        try {
            $sql = 'INSERT INTO project_stage(title, date_create) VALUES(:title, :date_create)';
            $this->_db->query($sql, array('title' => $this->_title, 'date_create' => $this->_dateCreate));

            $this->_id = $this->_db->lastInsertId('project_stage', 'id');
        } catch (Exception $e) {
    ...
    {
        try {
            $sql = 'DELETE FROM project_stage WHERE id=:id';
        ...
        try {
            ...
            if (isset($result[0])) {
                ...
                foreach ($result as $res) {
                    $retArray[] = SM_Project_Stage::getInstanceByArray($res);
                }
                return $retArray;
            } else {
                return false;
     ...
     * @param $id
     *
     * @return bool|SM_Project_Stage
     * @throws Exception
     */
    public static function getInstanceById($id)
    {
        try {
            $sql = 'SELECT * FROM project_stage WHERE id=:id';
